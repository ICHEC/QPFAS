...
class JSON(DB):
    def __init__(self, db_name : str = None, db_path : str = None):
        super(JSON, self).__init__(db_name, db_path)
        self.storage_fmt = "json"
    
    def store(self, obj):
        """ Write the class object to a specified JSON output file if an 
        output file is supplied as an argument. Returns the pickled json 
        string of the object.
        
        Args:
            obj (Object): Object to serialise and store.
            
        Returns:
            output (str): Pickled JSON string.
        """        

        encoded = jsonpickle.encode(obj, unpicklable=True)
        
        try:
            with open(self.db_full_path + ".json"), "w+") as fp:
                json.dump(encoded, fp)
        except:
            print("No file specified. Defaulting to out.json")
...
class SQLITE3(DB):
    def __init__(self, db_name : str = None, db_path : str = None):
        super(SQLITE3, self).__init__(db_name, db_path)
        self.storage_fmt = "sqlite3"

###############################################################################

    def create_table(self, table_name="qpfas"):
        """
        Create the database table.
        """
        cr_tbl = """CREATE TABLE {}(
                    id INTEGER PRIMARY KEY, mol_name TEXT, 
